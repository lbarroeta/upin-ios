...
class UserJoinedVC: UIViewController {
    ...
    func userListener() {
        listener = Firestore.firestore().collection("pins").document(selectedPin).collection("users").whereField("is_host", isEqualTo: false).addSnapshotListener({ (snapshot, error) in
            if let error = error {
            ...
            snapshot?.documentChanges.forEach({ (change) in
                ...
                let user = UserJoined.init(data: data)
...
extension UserJoinedVC: UITableViewDelegate, UITableViewDataSource {
    
    func onUserAdded(change: DocumentChange, user: UserJoined) {
        let newIndex = Int(change.newIndex)
        users.insert(user, at: newIndex)
        tableView.insertRows(at: [IndexPath.init(item: newIndex, section: 0)], with: .automatic)
    }
    
    func onUserModified(change: DocumentChange, user: UserJoined) {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: "UserJoinedCell", for: indexPath) as? UserJoinedCell {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "UserJoinedProfileVC", sender: self)
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "UserJoinedProfileVC" {
            if let destination = segue.destination as? UserJoinedProfileVC {
